...
: ${HOST:=${DB_PORT_5432_TCP_ADDR:='54.203.144.252'}}
: ${PORT:=${DB_PORT_5432_TCP_PORT:=5432}}
: ${USER:=${DB_ENV_POSTGRES_USER:=${POSTGRES_USER:='odoo'}}}
: ${PASSWORD:=${DB_ENV_POSTGRES_PASSWORD:=${POSTGRES_PASSWORD:='od00p4ss'}}}
...
then
	...
	sleep 5
	exec /usr/bin/odoo-server "--conf=$ODOO_RC" "--update=$UPGRADE_MODULE" "--database=$UPGRADE_DATABASE"
else
